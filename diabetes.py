...
class SurveyApp(QWidget):
    ...
    def ask_question(self, idx):
        ...
        # Build options safely (handles dict and string)
        options = []
        for v in options_dict.values():
            if isinstance(v, dict):
                options.append(v.get("answer", ""))
        ...
        if ok and choice:
            ...
            for val in options_dict.values():
                if isinstance(val, dict) and val.get("answer") == choice:
                    color = val.get("color", "G")
                    break
                elif isinstance(val, str) and val == choice:
                    color = "G"  # default for string answers

            # Save the answer and update chart
